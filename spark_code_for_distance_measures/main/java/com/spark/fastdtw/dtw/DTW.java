...
package com.spark.fastdtw.dtw;

import com.spark.fastdtw.matrix.ColMajorCell;
...
public class DTW implements Serializable
{
   ...
   public static double calcWarpCost(WarpPath path, TimeSeries tsI, TimeSeries tsJ, DistanceFunction distFn)
   {
      ...
      {
         final com.spark.fastdtw.matrix.ColMajorCell currWarp = path.get(p);
   ...
   public static double getMyDTWDist(TimeSeries tsI, TimeSeries tsJ, DistanceFunction distFn){
//	   File file = new File("log.txt");
//	   FileWriter fw = new FileWriter(file, true);
		double cost = 0.0;
		final int maxI = tsI.size();
		final int maxJ = tsJ.size();
...
//				fw.write("cost: " + cost + "\n");
		...
		//System.out.println("costMatrix[i][j] " +costMatrix[maxI][maxJ]);
//		fw.write("costMatrix[maxI][maxJ]/Math.max(maxI, maxJ) " + costMatrix[maxI][maxJ]/Math.max(maxI, maxJ));
//		fw.close();
		System.out.println("costMatrix[maxI][maxJ] " + costMatrix[maxI][maxJ] + " Math.max(maxI, maxJ) " + Math.max(maxI, maxJ));
		return costMatrix[maxI][maxJ];// for normalization use TimeWarpInfo
   }
   ...
   public static double getWarpDistBetween(TimeSeries tsI, DistanceFunction distFn)
   {
      // The space complexity is 2*tsJ.size().  Dynamic time warping is symmetric so switching the two time series
      //    parameters does not effect the final warp cost but can reduce the space complexity by allowing tsJ to
      //    be set as the shorter time series and only requiring 2 columns of size |tsJ| rather than 2 larger columns of
      ...
      // Calculate the values for the first column, from the bottom up.
      for (int i=0; i<=tsI.size()-1; i++)  // the rest of the first column
         dist += distFn.calcDistance(tsI.getMeasurementVector(i), new double [tsI.getMeasurementVector(i).length]);
      // Minimum Cost is at (maxI,maxJ)
      return dist;
   }  // end getWarpDistBetween(..)
